   ...
   QObject(parent)
{
    ...
    this->device=1;
    BASS_Init(device, 44100,0, 0, NULL); //iniciamos el motor de audio
...
void StreamRender::render(){
    ...
    Label->setText(w_StreamMath->SegundoToFormato(segundos-w_StreamMath->Trascurridos()));
           ...
           if(Encadenar){  //10 segundos de terminar la encadena con la siguiete
               if((int)w_StreamMath->Duracion()-10 <= (int)w_StreamMath->Trascurridos()){
                     Timer->stop();
                     emit  Finish();
                    return;
                 }
...
void StreamRender::setDevice(int device){
    this->device=device;

}

int StreamRender::Device(){
